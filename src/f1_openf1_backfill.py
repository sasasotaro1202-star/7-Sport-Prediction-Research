...
def get(path,params=None,timeout=45):
    r=requests.get(BASE+path,params=params or {},timeout=timeout,headers={'User-Agent':'SevenSportResearchEngine/4.6'}); r.raise_for_status(); return r.json(),r.url
def num(x):
    try:return float(x)
    except:return None
def main():
    ...
    for year in range(a.start_year,a.end_year+1):
        ...
        for s in sessions:
            ...
            for z in results:
                ...
                total+=1
            c.execute('INSERT OR REPLACE INTO source_snapshot(snapshot_id,source,source_url,retrieved_at_utc,source_available_at_utc,event_time_utc,parser_version,availability_status,provenance_json) VALUES(?,?,?,?,?,?,?,?,?)',(sid('openf1',sk), 'OpenF1',rurl,now,None,start,'openf1-v1','UNVERIFIABLE',json.dumps({'year':year,'session_key':sk})))
        c.commit()
    print({'f1_openf1_driver_session_rows':total}); c.close()
