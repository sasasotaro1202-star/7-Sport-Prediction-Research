...
The production F1 winner is intentionally gated because a true race winner
...
No post-race variables from the current race are used as features.
"""
...
def get_json(url: str) -> dict:
    for attempt in range(5):
        try:
            r = requests.get(
                url,
                headers={"User-Agent": "SevenSportResearchEngine/4.7"},
                timeout=30,
            )
            r.raise_for_status()
            return r.json()
...
def fetch_seasons(start_year=2018, end_year=None):
    ...
    for year in range(start_year, end_year + 1):
        payload = get_json(f"{BASE}/{year}/results.json?limit=1000")
        races.extend(payload.get("MRData", {}).get("RaceTable", {}).get("Races", []))
    return races
...
def build_pairwise_rows(races):
    ...
    for race in sorted(races, key=lambda r: (int(r.get("season", 0)), int(r.get("round", 0)))):
        ...
        for a, b in combinations(results, 2):
            ...
            ha, hb = list(finish_hist[da]), list(finish_hist[db])
            ca, cb = list(circuit_hist[da][circuit]), list(circuit_hist[db][circuit])
            fa, fb = list(dnf_hist[da]), list(dnf_hist[db])
            xa = [
                elo[da] - elo[db],
            ...
            out.append({
                "season": season, "round": rnd, "race": race_name, "circuit": circuit,
                "driver_a": db, "driver_b": da, "y": 1 - y,
                "x": [-v if np.isfinite(v) else np.nan for v in xa],
            })
        ...
        for rr in results:
            ...
            expected_w = sum(
                1.0 / (1.0 + 10.0 ** ((elo[z["driver"]] - elo[d]) / 400.0))
...
def main() -> int:
    ...
    if len(race_keys) < 20:
        payload = {
            "sport": "f1",
            "status": "DEFERRED",
            "reason": "insufficient_free_jolpica_race_history",
            "races": len(race_keys),
    ...
    for i in range(4, len(folds)):
        ...
        models = {
            "logistic": Pipeline([
                ("i", SimpleImputer(strategy="median", add_indicator=True, keep_empty_features=True)),
                ("s", StandardScaler()),
            ...
            "hist_gb": Pipeline([
                ...
                ("m", HistGradientBoostingClassifier(
                    max_iter=260, learning_rate=.04, l2_regularization=1.5,
                    max_leaf_nodes=15, random_state=43)),
            ]),
        }
    ...
    holdout = metric(yH, hp) if len(np.unique(yH)) > 1 else {
        "status": "INSUFFICIENT_LABEL_VARIATION", "n": len(yH)
    }
