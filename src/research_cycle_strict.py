...
from src import ultimate_predictive_control_v13 as ultimate_v13
...
def train(s):
    ...
    try:
     ...
     # v13 Ultimate Future-Generalization research layer. It consumes only
     # chronological OOF predictions already generated above; it never touches the
     # frozen holdout for selector fitting and never auto-promotes anything.
     try:
      ultimate_intelligence=ultimate_control.run_ultimate_research(
       sport=s,
       oof_folds=oof_folds,
       y=oof_y,
       event_ids=oof_event_ids,
       model_names=list(best),
       weights=candidate_weights,
       cutoff_utc=rows[sel-1][1],
       artifact_rel=f"results/research/{s}_ultimate_intelligence.json",
       feature_names=fs,
      )
      ultimate_control.write_artifact(
       ultimate_intelligence,
       RESULTS/f"{s}_ultimate_intelligence.json"
      )
     except Exception as exc:
      ultimate_intelligence={
       'sport':s,
       'status':'FAILED',
       'mode':'RESEARCH_ONLY',
       'reason':f'ultimate_research_exception:{type(exc).__name__}:{exc}',
       'promotion_status':'HOLD_RESEARCH_ONLY_NO_AUTO_PROMOTION',
      }
      RESULTS.mkdir(parents=True,exist_ok=True)
      (RESULTS/f'{s}_ultimate_intelligence.json').write_text(
       _json_dump(ultimate_intelligence,indent=2),encoding='utf-8'
      )
      # Fail closed: an auxiliary research failure must never be serialized as
      # a successful TRAINED run. Production artifacts are not mutated here.
      raise RuntimeError(ultimate_intelligence['reason']) from exc
     # ULTIMATE FINAL v13 causal layer: consume the exact chronological OOF
     ...
     try:
      ...
      ultimate_v13=ultimate_v13.run_v13_research_from_oof(
       sport=s,
