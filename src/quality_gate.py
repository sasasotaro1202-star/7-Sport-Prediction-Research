...
def main():
    ...
    if not DB.exists():
        ...
        checks.append({'check': 'sports_present', 'ok': not missing_sports, 'counts': {s: counts.get(s, 0) for s in scope_sports}, 'missing': missing_sports})
        if missing_sports:
        ...
        exact_without_availability = con.execute("select count(*) from source_snapshot where availability_status='EXACT' and source_available_at_utc is null").fetchone()[0]
        unver = con.execute("select count(*) from source_snapshot where availability_status='UNVERIFIABLE'").fetchone()[0]
        checks.append({'check': 'source_timing_transparency', 'ok': exact_without_availability == 0, 'exact_missing_source_available_at': int(exact_without_availability), 'unverifiable_source_timestamps': int(unver), 'policy': 'UNVERIFIABLE is excluded from strict PIT learning'})
        if exact_without_availability:
            fatal.append('exact_source_missing_availability_time')

        if 'f1' in scope_sports:
            bad_f1_roles = con.execute("select count(*) from event_participant ep join event e on e.event_id=ep.event_id where e.sport='f1' and lower(coalesce(ep.role,'')) <> 'driver'").fetchone()[0]
        ...
        leak = con.execute("select count(*) from pit_replay where leakage_status not in ('PASS','UNKNOWN','CLEAN')").fetchone()[0]
        exact = con.execute("select count(*) from pit_replay where replay_status='EXACT' and leakage_status in ('PASS','CLEAN')").fetchone()[0]
        ...
        outcomes = {k: int(v) for k, v in con.execute("select sport,count(*) from event_outcome where outcome_status='VERIFIED' group by sport").fetchall()}
        missing_outcomes = [s for s in scope_sports if outcomes.get(s, 0) == 0]
        checks.append({'check': 'verified_outcomes', 'ok': not missing_outcomes, 'counts': {s: outcomes.get(s, 0) for s in scope_sports}, 'missing': missing_outcomes})
        if missing_outcomes:
            pending.append('verified_outcome_coverage_incomplete')

        model_rows = con.execute("select sport,count(*) from model_state_snapshot where quality_status like 'ACCEPTED%' group by sport").fetchall()
        models = {k: int(v) for k, v in model_rows}
        missing_models = [s for s in scope_sports if models.get(s, 0) == 0]
        checks.append({'check': 'accepted_models', 'ok': not missing_models, 'counts': {s: models.get(s, 0) for s in scope_sports}, 'missing': missing_models})
        if missing_models:
    ...
    report = {'timestamp_utc': now(), 'status': status, 'fatal': fatal, 'pending': sorted(set(pending)), 'checks': checks, 'strict': a.strict, 'scope': a.sport or 'global', 'policy': {'fatal_integrity_errors_block_publish': True, 'coverage_pending_does_not_block_hourly_pipeline': not a.strict, 'strict_mode_requires_full_coverage_and_models': True}}
