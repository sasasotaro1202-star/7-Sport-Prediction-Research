...
def build_matchday_change_context_rows(
    ...
    """Build PIT-safe T-60 state plus the immediate T-90 -> T-60 change.

    The temporal signal focuses on the final pre-match transition:
    availability, lineup confirmation, weather/market/news evidence and
    freshness changes. Missing horizon evidence remains NaN and never becomes
    a synthetic zero.
    """
    horizons = tuple(sorted({int(x) for x in lead_minutes}, reverse=True))
    if horizons != (90, 60):
        raise ValueError("lead_minutes must be exactly (90, 60)")
    matrices = {
    ...
    earlier = matrices[90]
    cols = [0, 2, 4, 5, 6, 13]
    immediate = np.column_stack([latest[:, c] - earlier[:, c] for c in cols])
    finite = np.isfinite(latest[:, cols]) & np.isfinite(earlier[:, cols])
    immediate = np.where(finite, immediate, np.nan)
    return np.column_stack([latest, immediate]).tolist()
