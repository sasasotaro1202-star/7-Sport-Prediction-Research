...
def fetch_games(league_id):
    ...
    r=requests.get(live,headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0 Safari/537.36','Accept':'application/json, text/plain, */*','Origin':'https://www.nba.com','Referer':'https://www.nba.com/'},timeout=20); r.raise_for_status(); return live, r.json().get('scoreboard',{}).get('games',[]) or []
...
def collect(league_id):
    ...
    for raw in games:
        ...
        for i,t in enumerate((h,a)):
            ...
            c.execute('''INSERT OR REPLACE INTO event_participant(event_id,participant_id,team_id,side,source,source_url,quality_status) VALUES(?,?,?,?,?,?,?)''',(eid,pid,pid,'A' if i==0 else 'B','data.nba.com',url,'UNVERIFIABLE'))
            if t.get('score') not in (None,''):
                try: num=float(t.get('score'))
                except Exception: num=None
                c.execute('''INSERT OR REPLACE INTO match_stats(stat_id,event_id,participant_id,team_id,sport,observed_at_utc,stat_name,value_num,value_text,source,source_url,quality_status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(sid(eid,pid,'score',t.get('score')),eid,pid,pid,SPORT,now,'score',num,str(t.get('score')),'data.nba.com',url,'UNVERIFIABLE'))
        ph=hashlib.sha256(json.dumps(raw,sort_keys=True,default=str).encode()).hexdigest(); c.execute('''INSERT OR REPLACE INTO source_snapshot(snapshot_id,source,source_url,retrieved_at_utc,event_time_utc,content_hash,parser_version,availability_status,provenance_json) VALUES(?,?,?,?,?,?,?,?,?)''',(sid('data.nba.com',league_id,gid),'data.nba.com',url,now,et,ph,'basketball-v4.5.13','UNVERIFIABLE',json.dumps({'sport':SPORT,'league':league},ensure_ascii=False)))
