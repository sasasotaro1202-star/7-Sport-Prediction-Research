...
"""Research-only online fixed-share Hedge for probabilistic expert fusion.

All weight updates occur strictly after the scored observation. Equal-timestamp
events are treated as an atomic information block so one realized outcome can
never affect another prediction at the same timestamp.
"""
...
class FixedShareHedge:
    ...
    def predict(self, probabilities: Sequence[float]) -> float:
        ...
        p=sum(w*_clip(x) for w,x in zip(self.weights,probabilities))
        return _clip(p)
...
def chronological_predictions(events: Iterable[HedgeEvent]) -> list[tuple[HedgeEvent,float]]:
    """Generate predictions causally, updating only between timestamp blocks."""
