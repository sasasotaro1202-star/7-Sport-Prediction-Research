...
def fast_build_rows(c,sport):
    ...
    event_ids=[x[0] for x in events]; ph=','.join('?' for _ in event_ids)
    outcomes={eid:outcome for eid,outcome in c.execute(f"SELECT event_id,outcome FROM event_outcome WHERE outcome_status='VERIFIED' AND event_id IN ({ph})",event_ids).fetchall() if outcome in ('A','B')}
    participants={}
    ...
    for eid,et in events:
        ...
        if outcome not in ('A','B') or len(ps)!=2: continue
        feat={}
        for pid,side in ps:
            for stat in cols:
                vals=[]
...
if __name__=='__main__':
    sys.argv[0]='research_cycle.py'; rc.main()
