...
def fetch(url, timeout=20):
    r = requests.get(url, headers={'User-Agent':'SevenSportResearchEngine/Rugby-v1','Accept-Language':'en-US,en;q=0.8,ja;q=0.6'}, timeout=timeout)
    r.raise_for_status()
...
def collect(max_pages):
    ...
    while queue and pages < max_pages:
        ...
        for item in jsonld(html):
            ...
            events += 1; found=True; snapshots += 1
        soup=BeautifulSoup(html,'lxml')
...
if __name__=='__main__':
    ...
    args=ap.parse_args()
    raise SystemExit(collect(args.max_pages))
