...
class TemporalSigmoidCalibrated:
    """Fit base model on an older prefix and Platt calibrator on its later suffix only."""
    def __init__(self,base,calibration_fraction=.20):
        self.base=base; self.calibration_fraction=calibration_fraction; self.calibrator=None
    def fit(self,X,y):
        n=len(y); split=max(20,int(n*(1-self.calibration_fraction)))
        if split>=n or len(np.unique(y[:split]))<2:
            self.base.fit(X,y); self.calibrator=None; return self
        self.base.fit(X[:split],y[:split]); raw=self.base.predict_proba(X[split:])[:,1]; yy=np.asarray(y[split:],dtype=int)
        if len(np.unique(yy))<2:
            self.calibrator=None; return self
        eps=1e-6; z=np.log(np.clip(raw,eps,1-eps)/(1-np.clip(raw,eps,1-eps))).reshape(-1,1)
        self.calibrator=LogisticRegression(C=1.0,max_iter=1000); self.calibrator.fit(z,yy); return self
...
def train_sport(sport):
    ...
    selection_end=max(60,int(len(rows)*.78)); holdout_n=len(rows)-selection_end
    ...
    pool=model_pool(); selection_results=[]; step=max(10,min(30,selection_end-max(50,int(selection_end*.70))))
    for name in pool:
        ...
        for end in range(max(50,int(selection_end*.70)),selection_end,step):
            ...
            model=model_pool()[name]; model.fit(X[:end],y[:end]); p=model.predict_proba(X[end:test_end])[:,1]; probs.extend(p.tolist()); truth.extend(y[end:test_end].tolist()); preds.extend((p>=.5).astype(int).tolist()); folds.append({'train_end':rows[end-1][1],'test_start':rows[end][1],'test_end':rows[test_end-1][1],'n':test_end-end})
    ...
    if not selection_results:
        c.close(); return {'sport':sport,'status':'DEFERRED','reason':'no_valid_selection_folds','rows':len(rows)}
    selection_results.sort(key=lambda z:(z['logloss'],z['brier'],z['ece'])); best_name=selection_results[0]['model']
    holdout_model=model_pool()[best_name]; holdout_model.fit(X[:selection_end],y[:selection_end]); holdout_prob=holdout_model.predict_proba(X[selection_end:])[:,1]; holdout=score(y[selection_end:],holdout_prob); holdout['model']=best_name
    if holdout['ece']>0.20:
    ...
    inc=incumbent(c,sport); accept=True; gate_reason='initial_model_locked_holdout'
    old_hold=inc['metadata'].get('holdout_metrics') if inc else None
    if old_hold:
        oldll=float(old_hold.get('logloss',1e9)); newll=float(holdout['logloss']); tol=max(.002,0.01*oldll); accept=newll <= oldll-tol; gate_reason=f'challenger_locked_holdout_logloss={newll:.6f}; incumbent_locked_holdout_logloss={oldll:.6f}; required_improvement={tol:.6f}'
    if not accept:
    ...
    final=model_pool()[best_name]; final.fit(X,y); version=h({'sport':sport,'features':features,'model':best_name,'rows':len(rows),'last_event':rows[-1][1],'selection':selection_results[0],'holdout':holdout}); MODELS.mkdir(parents=True,exist_ok=True); RESULTS.mkdir(parents=True,exist_ok=True); artifact=MODELS/f'{sport}_{version}.joblib'; joblib.dump({'model':final,'features':features,'sport':sport,'model_version':version,'training_rows':len(rows)},artifact); gitsha=subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,text=True,capture_output=True).stdout.strip() or None
    metadata={'sport':sport,'market':'winner','model_version':version,'feature_version':'strict-pit-v7-locked-holdout-temporal-calibration-robust-features','training_cutoff_utc':rows[-1][1],'git_commit_sha':gitsha,'artifact_path':str(artifact.relative_to(ROOT)),'quality_status':'ACCEPTED_LOCKED_HOLDOUT','selection_gate':gate_reason,'selection_metrics':selection_results,'holdout_metrics':holdout}
    c.execute('''INSERT INTO model_state_snapshot(snapshot_id,sport,market,as_of_utc,model_version,feature_version,training_cutoff_utc,dataset_hash,git_commit_sha,artifact_path,quality_status,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(h(metadata),sport,'winner',utc(),version,metadata['feature_version'],rows[-1][1],h([(r[0],r[1],r[2]) for r in rows]),gitsha,str(artifact.relative_to(ROOT)),'ACCEPTED_LOCKED_HOLDOUT',json.dumps(metadata,ensure_ascii=False))); c.commit(); c.close()
    out={'sport':sport,'status':'TRAINED','model':best_name,'model_version':version,'training_rows':len(rows),'features':len(features),'selection_metrics':selection_results,'holdout_metrics':holdout,'selection_gate':gate_reason}; (RESULTS/f'{sport}.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8'); return out
