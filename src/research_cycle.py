...
def model_pool(seed=42):
    base={
      'logistic':Pipeline([('imp',SimpleImputer(strategy='median')),('scale',StandardScaler()),('model',LogisticRegression(max_iter=3000,C=1.0,random_state=seed))]),
      'extra_trees':Pipeline([('imp',SimpleImputer(strategy='median')),('model',ExtraTreesClassifier(n_estimators=500,min_samples_leaf=3,max_features='sqrt',random_state=seed,n_jobs=-1,class_weight='balanced'))]),
      'random_forest':Pipeline([('imp',SimpleImputer(strategy='median')),('model',RandomForestClassifier(n_estimators=500,min_samples_leaf=3,max_features='sqrt',random_state=seed,n_jobs=-1,class_weight='balanced'))]),
      'hist_gb':Pipeline([('imp',SimpleImputer(strategy='median')),('model',HistGradientBoostingClassifier(max_iter=350,learning_rate=.035,l2_regularization=.75,random_state=seed))])}
...
def build_rows(c,sport):
    ...
    for eid,et in events:
        ...
        ps=c.execute("SELECT participant_id,side FROM event_participant WHERE event_id=? AND side IN ('A','B') GROUP BY participant_id,side ORDER BY side",(eid,)).fetchall()
        ...
        for pid,side in ps:
            for stat in cols:
                vals=c.execute("""SELECT ms.value_num FROM match_stats ms JOIN event pe ON pe.event_id=ms.event_id
                                  ...
                                  AND pe.event_time_utc < ? AND ms.value_num IS NOT NULL ORDER BY pe.event_time_utc DESC,ms.stat_id DESC LIMIT 20""",(sport,pid,stat,et)).fetchall()
                x=np.asarray([v[0] for v in vals],dtype=float)
                feat[f'{side}__{stat}__n']=float(len(x)); feat[f'{side}__{stat}__mean']=float(x.mean()) if len(x) else np.nan
                feat[f'{side}__{stat}__last']=float(x[0]) if len(x) else np.nan; feat[f'{side}__{stat}__std']=float(x.std()) if len(x)>1 else np.nan
...
def train_sport(sport):
    ...
    if len(rows)<80 or len(features)<2:
        out={'sport':sport,'status':'DEFERRED','reason':'insufficient_verified_PIT_training_rows','rows':len(rows),'features':len(features),'timestamp_utc':utc()}; c.close(); return out
    X=np.array([[r[3].get(f,np.nan) for f in features] for r in rows],dtype=float); y=np.array([r[2] for r in rows],dtype=int)
    split=max(50,int(len(rows)*.70)); pool=model_pool(); results=[]
    step=max(10,min(30,len(rows)-split))
    for name,proto in pool.items():
        ...
        for end in range(split,len(rows),step):
            ...
            model=proto; model.fit(X[:end],y[:end]); p=model.predict_proba(X[end:test_end])[:,1]; z=(p>=.5).astype(int)
            probs.extend(p.tolist()); truth.extend(y[end:test_end].tolist()); preds.extend(z.tolist())
            folds.append({'train_end':rows[end-1][1],'test_start':rows[end][1],'test_end':rows[test_end-1][1],'n':test_end-end})
        if len(truth)<20 or len(set(truth))<2: continue
        ll=log_loss(truth,np.column_stack([1-np.asarray(probs),np.asarray(probs)]),labels=[0,1]); br=brier_score_loss(truth,probs); acc=accuracy_score(truth,preds); ec=ece(np.asarray(truth),np.asarray(probs))
        results.append({'model':name,'logloss':float(ll),'brier':float(br),'accuracy':float(acc),'ece':float(ec),'oos_n':len(truth),'folds':folds})
    if not results:
        c.close(); return {'sport':sport,'status':'DEFERRED','reason':'no_valid_oos_folds','rows':len(rows)}
    results.sort(key=lambda z:(z['logloss'],z['brier'],z['ece']))
    best=results[0]
    # A challenger is accepted only when it has enough OOS evidence and is not materially worse than the incumbent.
    inc=incumbent(c,sport); accept=True; gate_reason='initial_model'
    if inc:
        old=inc['metadata'].get('metrics',[]); oldbest=min(old,key=lambda z:z.get('logloss',1e9)) if old else None
        if oldbest:
            oldll=float(oldbest.get('logloss',1e9)); newll=float(best['logloss']); tol=max(.002,0.01*oldll)
            accept=newll <= oldll-tol
            gate_reason=f"challenger_logloss={newll:.6f}; incumbent_logloss={oldll:.6f}; required_improvement={tol:.6f}"
    ...
    final=pool[best['model']]; final.fit(X,y)
    version=h({'sport':sport,'features':features,'model':best['model'],'rows':len(rows),'last_event':rows[-1][1],'metrics':best})
    ...
    metadata={'sport':sport,'market':'winner','model_version':version,'feature_version':'pit-v2-calibrated','training_cutoff_utc':rows[-1][1],'git_commit_sha':gitsha,'artifact_path':str(artifact.relative_to(ROOT)),'quality_status':'ACCEPTED_AFTER_OOS','selection_gate':gate_reason,'metrics':results}
    c.execute('''INSERT INTO model_state_snapshot(snapshot_id,sport,market,as_of_utc,model_version,feature_version,training_cutoff_utc,dataset_hash,git_commit_sha,artifact_path,quality_status,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(h(metadata),sport,'winner',utc(),version,'pit-v2-calibrated',rows[-1][1],h([(r[0],r[1],r[2]) for r in rows]),gitsha,str(artifact.relative_to(ROOT)),'ACCEPTED_AFTER_OOS',json.dumps(metadata,ensure_ascii=False)))
    c.commit(); c.close()
    out={'sport':sport,'status':'TRAINED','model':best['model'],'model_version':version,'training_rows':len(rows),'features':len(features),'metrics':results,'selection_gate':gate_reason}
...
def main():
    ...
    print(json.dumps([train_sport(s) for s in sports],ensure_ascii=False,indent=2))
