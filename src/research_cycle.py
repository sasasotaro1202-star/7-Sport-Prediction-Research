...
def build_rows(c,sport):
    ...
    for eid,et in events:
        ...
        for pid,side in ps:
            for stat in cols:
                # Strict PIT: a historical observation is eligible only when its effective/source-availability
                # timestamp is explicitly known and is at or before this match's prediction cutoff.
                vals=c.execute("""SELECT ms.value_num FROM match_stats ms JOIN event pe ON pe.event_id=ms.event_id
                                  ...
                                  ORDER BY pe.event_time_utc DESC,ms.stat_id DESC LIMIT 20""",(sport,pid,stat,et,cutoff)).fetchall()
        ...
        # A row is retained only when at least one real PIT feature exists on each side.
...
def train_sport(sport):
    ...
    results.sort(key=lambda z:(z['logloss'],z['brier'],z['ece'])); best=results[0]
    ...
    if inc:
        ...
        if oldbest:
            oldll=float(oldbest.get('logloss',1e9)); newll=float(best['logloss']); tol=max(.002,0.01*oldll); accept=newll <= oldll-tol
    ...
    metadata={'sport':sport,'market':'winner','model_version':version,'feature_version':'strict-pit-v3-calibrated','training_cutoff_utc':rows[-1][1],'git_commit_sha':gitsha,'artifact_path':str(artifact.relative_to(ROOT)),'quality_status':'ACCEPTED_AFTER_OOS','selection_gate':gate_reason,'metrics':results}
    c.execute('''INSERT INTO model_state_snapshot(snapshot_id,sport,market,as_of_utc,model_version,feature_version,training_cutoff_utc,dataset_hash,git_commit_sha,artifact_path,quality_status,metadata_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(h(metadata),sport,'winner',utc(),version,'strict-pit-v3-calibrated',rows[-1][1],h([(r[0],r[1],r[2]) for r in rows]),gitsha,str(artifact.relative_to(ROOT)),'ACCEPTED_AFTER_OOS',json.dumps(metadata,ensure_ascii=False)))
    c.commit(); c.close()
