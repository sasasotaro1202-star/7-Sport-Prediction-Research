...
def http(url: str, timeout: int = 30):
    headers = {"User-Agent": "SevenSportResearchEngine/4.5.14"}
    try:
...
def backfill_csv_games(c, url, sport, competition):
    ...
    for row in rows:
        ...
        et = iso(date)
        eid = upsert_event(c, sport, f"{home} vs {away}", et, "public-dataset", url, "COMPLETED", competition=competition, season=str(date)[:4] if date else None)
...
def backfill_rizin(c, max_pages=100):
    ...
    for url in urls[:max_pages]:
        ...
        for a, b in pairs:
            ...
            # Prefer the explicit local WIN/LOSE marker around the pair.
            segment = text[max(0, text.find(a)-80):text.find(b)+120]
            side = "A" if re.search(r"\(WIN\).*?" + re.escape(a), segment, re.I) else "B"
    ...
    while queue and len(seen) < max_pages:
        ...
        for link in links:
            if re.search(r"/matches?/|/competitions/", link, re.I) and link not in seen:
                queue.append(link)
        # JSON-LD-like payloads are present on some competition pages.
...
def main():
    ...
    try:
        for sport in [x.strip() for x in a.sports.split(",") if x.strip()]:
            try:
                if sport == "valorant":
                    ...
                    try:
                        out["nba"] = backfill_csv_games(c, NBA_GAMES, "basketball", "NBA")
                    except Exception as e:
                        warnings.append({"source": "nba", "error": repr(e)})
                    try:
                        out["wnba"] = backfill_csv_games(c, WNBA_GAMES, "basketball", "WNBA")
                    except Exception as e:
                        warnings.append({"source": "wnba", "error": repr(e)})
                    try:
                        out["fiba_world_cup_2019"] = backfill_csv_games(c, FIBA_2019, "basketball", "FIBA World Cup 2019")
                    except Exception as e:
                        warnings.append({"source": "fiba_world_cup_2019", "error": repr(e)})
                elif sport == "rizin":
        ...
        c.close()
    report = {"parser_version": "v4.5.14-public-history", "added": out, "warnings": warnings, "timestamp_utc": utcnow()}
    ...
    # Public fallback availability is intentionally non-blocking. The quality
    # gate remains the authoritative acceptance mechanism and records warnings.
    raise SystemExit(0)
