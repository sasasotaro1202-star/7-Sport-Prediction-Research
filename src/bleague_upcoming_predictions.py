...
def parse_detail(html: str, url: str):
    ...
    away = re.sub(r'\s+(B\.PREMIER|B\.ONE|B\.NEXT)\s* or not away or home == away:
        return None
...
def main():
    ...
    try:
        ...
        for key in list(range(506406, 506430)) + list(range(507183, 507207)) + list(range(507911, 507916)):
...
if __name__ == '__main__':
    main(), '', away).strip()\n    home = re.sub(r'\s*\|\s*B\.LEAGUE（Bリーグ）公式サイト
...
if __name__ == '__main__':
    main(), '', home).strip()\n    away = re.sub(r'\s*\|\s*B\.LEAGUE（Bリーグ）公式サイト
    if not home or not away or home == away:
        return None
    return {'date': dm.group(1), 'time': tm.group(1) if tm else None, 'home': home, 'away': away,
            'competition': compm.group(1) if compm else 'B.LEAGUE', 'url': url}
